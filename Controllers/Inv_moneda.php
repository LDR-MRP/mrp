            ...
            if (
                ...
                if ($intidmoneda == 0) {
                    ...
                    if ($_SESSION['permisosMod']['w']) {
                        $request_moneda = $this->model->inserMoneda($cve_moneda, $descripcion, $simbolo, $fecha_creacion, $estado);
                        $option = 1;
                    }
                } else {
                    //Actualizar
                    if ($_SESSION['permisosMod']['u']) {
                        $request_moneda = $this->model->updateMoneda($intidmoneda, $cve_moneda, $descripcion, $simbolo, $estado);
                        $option = 2;
                    }
                }
                if ($request_moneda === "exist") {

                    $arrResponse = array(
                        'status' => false,
                        'msg' => '¡Atención! ya existe.'
                    );
                } else if ($request_moneda !== false) {
