<?php
class Plan_confproductosv1 extends Controllers
{
	...
	public function Plan_confproductosv1()
	{
		...
		// $data['page_name'] = "";
		$data['page_functions_js'] = "functions_plan_confproductosv1.js";
		$this->views->getView($this, "plan_confproductosv1", $data);
	}
		...
		if ($_POST) {
			if (
				...
				if ($intIdproducto == 0) {
					...
					$request_producto = $this->model->inserProducto($claveUnica, $inventarioid, $lineaproductoid, $descripcion, $fecha_creacion, $estado, $plantaid);
					$option = 1;
					// }
		...
		try {
			...
			if ($idruta <= 0) {
				...
				foreach ($detalle as $row) {
					...
					$calidad = (int) ($row['calidad'] ?? 0);


					if ($idestacion > 0 && $orden > 0) {
						$request_detalle = $this->model->insertRutaDetalle($idruta, $idestacion, $orden, $now, $estampado, $calidad);
			...
			foreach ($detalle as $row) {
				...
				if ($iddetalle > 0) {
					$this->model->updateRutaDetalle($iddetalle, $idestacion, $orden, $estampado, $calidad);
					$idsDetalleVistos[] = $iddetalle;
					continue;
				}
...
public function getPdiCompletoEstacion($idestacion)
{
	...
	public function setComponentesSubensamble()
	{
		...
		if (!isset($_POST['componentes'])) {
			echo json_encode([
				'status' => false,
				'msg' => 'No llegó el payload componentes'
			], JSON_UNESCAPED_UNICODE);
			exit;
		}
		...
		if (json_last_error() !== JSON_ERROR_NONE || !is_array($payload) || empty($payload[0])) {
			echo json_encode([
				'status' => false,
				'msg' => 'JSON inválido en componentes'
			], JSON_UNESCAPED_UNICODE);
			exit;
		}
		...
		if (!$idAlmacen || !$idProducto || !$idSubensamble) {
			echo json_encode([
				'status' => false,
				'msg' => 'Faltan datos: idalmacen/idproducto/idsubensamble'
			], JSON_UNESCAPED_UNICODE);
			exit;
		}
		...
		$existentes = $this->model->selectComponentesSubensambleAllEstados($idSubensamble, $idProducto, $idAlmacen);
		$existMap = [];
		...
		foreach ($incoming as $inventarioid => $cantidad) {
			if (isset($existMap[$inventarioid])) {
				$idsubcomponente = $existMap[$inventarioid];

				$this->model->updateComponenteSubensamble($idsubcomponente, $cantidad, 2);

				$this->model->insertAuditoria(
					MPCONFPRODUCTOS,
					2,
					$idusuario,
					'mrp_subensamble_componentes',
					$idsubcomponente,
					$fechaEvento,
					$ip,
					$detalleAudit
				...
				$request_componentes = $this->model->insertComponenteSubensamble(
					$idAlmacen,
					$idProducto,
					$idSubensamble,
					$inventarioid,
		...
		$idsIncoming = array_keys($incoming);
		$this->model->softDeleteComponentesSubensambleNoIncluidos($idAlmacen, $idProducto, $idSubensamble, $idsIncoming);

		echo json_encode([
			'status' => true,
			'msg' => 'Componentes del subensamble sincronizados correctamente'
		], JSON_UNESCAPED_UNICODE);
		exit;
	}
		...
		public function getComponentesSubensamble($idsubensamble)
	{
		...
		if ($idsubensamble <= 0 || $idproducto <= 0) {
			echo json_encode([
				'status' => false,
				'msg' => 'Faltan datos: idsubensamble/idproducto'
			], JSON_UNESCAPED_UNICODE);
			exit;
		}

		$rows = $this->model->selectComponentesSubensamble($idsubensamble, $idproducto);
		...
		public function setHerramientasSubensamble()
	{
		...
		if (!isset($_POST['herramientas'])) {
			echo json_encode([
				'status' => false,
				'msg' => 'No llegó el payload herramientas'
			], JSON_UNESCAPED_UNICODE);
			exit;
		}
		...
		if (json_last_error() !== JSON_ERROR_NONE || !is_array($payload) || empty($payload[0])) {
			echo json_encode([
				'status' => false,
				'msg' => 'JSON inválido en herramientas'
			], JSON_UNESCAPED_UNICODE);
			exit;
		}
		...
		$existentes = $this->model->selectHerramientasSubensambleAllEstados($idSubensamble, $idProducto, $idAlmacen);
		$existMap = [];
		...
		foreach ($incoming as $inventarioid => $cantidad) {
			if (isset($existMap[$inventarioid])) {
				$idsubherramienta = $existMap[$inventarioid];

				$this->model->updateHerramientaSubensamble($idsubherramienta, $cantidad, 2);

				$this->model->insertAuditoria(
					MPCONFPRODUCTOS,
					2,
					$idusuario,
					'mrp_subensamble_herramientas',
					$idsubherramienta,
					$fechaEvento,
					$ip,
					$detalleHerramienta
				...
				$request_herramienta = $this->model->insertHerramientaSubensamble(
					$idAlmacen,
					$idProducto,
					$idSubensamble,
					$inventarioid,
		...
		$idsIncoming = array_keys($incoming);
		$this->model->softDeleteHerramientasSubensambleNoIncluidos($idAlmacen, $idProducto, $idSubensamble, $idsIncoming);

		echo json_encode([
			'status' => true,
			'msg' => 'Herramientas del subensamble sincronizadas correctamente'
		], JSON_UNESCAPED_UNICODE);
		exit;
	}
		...
		public function getHerramientasSubensamble($idsubensamble)
	{
		...
		$rows = $this->model->selectHerramientasSubensamble($idsubensamble, $idproducto);
