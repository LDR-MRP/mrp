...
class Rpt_mrp_planeacion extends Controllers
{
    ...
    public function exportExcel()
    {
        ...
        $sheet->setCellValue('B4', $info['num_orden'] ?? '—');
        $sheet->setCellValue('A5', 'Producto');
        $sheet->setCellValue('B5', $info['producto'] ?? '—');
        $sheet->setCellValue('A6', 'Supervisor');
        $sheet->setCellValue('B6', $info['supervisor'] ?? '—');
        $sheet->setCellValue('A7', 'Generado');
        $sheet->setCellValue('B7', date('Y-m-d H:i:s'));
    ...
    public function exportPdf()
    {
        ...
        ob_start();
        $d = $data;
        $this->views->getView($this, "pdf_kpi_ejecutivo", $d);
        $html = ob_get_clean();
