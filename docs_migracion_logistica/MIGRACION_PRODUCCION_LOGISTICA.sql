...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. TRASLADISTAS (PROVEEDORES), MADRINAS Y CHOFERES
-- ─────────────────────────────────────────────────────────────────────────────

-- 2.1 Actividad de Proveedor Trasladista y Tabla Relacional
...
-- 2.2 Registro de los 19 Proveedores Trasladistas
...
-- 2.3 Vincular trasladistas con la actividad 'TRASLADO_UNIDADES'
...
-- 2.4 Flota de Madrinas y Choferes
...
-- 2.5 Matriz de Tarifas Personalizadas por Proveedor
...
CREATE TABLE IF NOT EXISTS `lgs_costos_rutas` (
    ...
    `id_proveedor`     BIGINT UNSIGNED NULL DEFAULT NULL,
    `num_vins_min`     INT NOT NULL DEFAULT 1,
...
-- Asegurar columna id_proveedor en lgs_costos_rutas si la tabla ya existía
SET @col_prov_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'lgs_costos_rutas' AND COLUMN_NAME = 'id_proveedor');
SET @query_prov = IF(@col_prov_exists = 0, 'ALTER TABLE `lgs_costos_rutas` ADD COLUMN `id_proveedor` BIGINT UNSIGNED NULL DEFAULT NULL AFTER `id_segmento`, ADD KEY `idx_costo_ruta_prov` (`id_proveedor`)', 'SELECT 1');
PREPARE stmt_prov FROM @query_prov;
EXECUTE stmt_prov;
DEALLOCATE PREPARE stmt_prov;
