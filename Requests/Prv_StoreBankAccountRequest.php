...
class Prv_StoreBankAccountRequest extends Requests
{
    ...
    private function validateBasicRules(): void
    {
        ...
        if ($esNacional) {
            ...
            if ($idMoneda && !$hasSwift && !$hasIban) {
                $this->addError('swift_bic', 'Debe proporcionar el SWIFT/BIC o el IBAN para transferencias internacionales.');
            }

            if ($idMoneda && $hasSwift && !preg_match('/^[A-Z0-9]{8,11}$/i', $this->data['swift_bic'])) {
                $this->addError('swift_bic', 'El formato del código SWIFT / BIC es inválido.');
            }
        }
