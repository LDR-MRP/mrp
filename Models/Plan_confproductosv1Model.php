...
//     $sql = "SELECT *
//             FROM mrp_estacion_operaciones
//             WHERE estacionid = {$idestacion}
//               AND productoid = {$idproducto}
//               AND estado = 2
//             ORDER BY orden ASC, idoperacion ASC";
...
public function syncHerramientasSubensamble($idsubensamble, $idalmacen, $detalle = [])
{
    ...
    foreach ($detalle as $row) {
        ...
        if ($inventarioid <= 0 || $cantidad <= 0) continue;
...
public function getOrCreatePdiCabecera($idproducto, $idestacion, $fecha)
{
    ...
    $query = "INSERT INTO mrp_estacion_pdi(productoid, estacionid, fecha_creacion, estado)
              VALUES(?,?,?,?)";
    $arrData = [$idproducto, $idestacion, $fecha, 2];
    return (int)$this->insert($query, $arrData);
}

public function insertPdiZona($idpdi, $nombreZona, $referencia, $orden, $fecha)
{
    $query = "INSERT INTO mrp_estacion_pdi_zona(pdiid, nombre_zona, referencia, orden, estado, fecha_creacion)
              VALUES(?,?,?,?,?,?)";
    $arrData = [
        (int)$idpdi,
        $nombreZona,
...
public function updatePdiZona($idzona, $nombreZona, $referencia, $orden)
{
    $sql = "UPDATE mrp_estacion_pdi_zona
            SET nombre_zona = ?, referencia = ?, orden = ?, estado = 2
            WHERE idzona = ?";
    $arrData = [$nombreZona, $referencia, (int)$orden, (int)$idzona];
    return $this->update($sql, $arrData);
}

public function insertPdiPunto($idzona, $punto, $orden, $checkChina, $checkMexico, $checkI1, $checkI2, $checkI3, $checkI4, $fecha)
{
    $query = "INSERT INTO mrp_estacion_pdi_punto(
                zonaid, punto, orden, check_china, check_mexico, check_i1, check_i2, check_i3, check_i4, estado, fecha_creacion
              ) VALUES(?,?,?,?,?,?,?,?,?,?,?)";
...
public function updatePdiPunto($idpuntopdi, $punto, $orden, $checkChina, $checkMexico, $checkI1, $checkI2, $checkI3, $checkI4)
{
    $sql = "UPDATE mrp_estacion_pdi_punto
            SET punto = ?, orden = ?, check_china = ?, check_mexico = ?, check_i1 = ?, check_i2 = ?, check_i3 = ?, check_i4 = ?, estado = 2
            WHERE idpuntopdi = ?";
