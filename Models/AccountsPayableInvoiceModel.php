...
class AccountsPayableInvoiceModel extends Mysql
{
    ...
    /**
     * Obtiene las facturas autorizadas para pago (estatus_validacion = 1) 
     * que siguen pendientes, junto con la cuenta bancaria principal aprobada del proveedor.
     */
    public function getPendingPayments(): array
    {
        $query = "SELECT 
                    ...
                    cb.id_banco,
                    cb.clabe AS cuenta_clabe,
                    b.nombre_corto AS nombre_banco
                  FROM {$this->table} f
                  INNER JOIN prv_cat_proveedores p ON p.id_proveedor = f.id_proveedor
                  -- Traemos únicamente la cuenta bancaria principal aprobada del proveedor (Antifraude)
                  INNER JOIN prv_det_cuentas_bancarias cb ON cb.id_proveedor = p.id_proveedor 
                    AND cb.es_principal = 1 
                    AND cb.estatus_aprobacion = 'APROBADO'
                    AND cb.deleted_at IS NULL
                  LEFT JOIN cat_bancos b ON cb.id_banco = b.id_banco
                  WHERE f.estatus_validacion = 1 -- 1 = Autorizada por 3-Way Match
                    AND f.estatus_pago = 'PENDIENTE'
                    AND f.deleted_at IS NULL
    ...
    /**
     * Actualiza de forma masiva el estado de pago de las facturas incluidas en el Layout a "PROGRAMADO".
     * 
     * @param array $invoiceIds Array de IDs de facturas
     * @param int   $userId     ID del administrador que realiza la acción
     */
    public function markAsProgrammed(array $invoiceIds, int $userId): bool
    {
        // Generamos los placeholders "?" dinámicamente según la cantidad de IDs
        $placeholders = implode(',', array_fill(0, count($invoiceIds), '?'));
        
        $query = "UPDATE {$this->table} 
                  SET estatus_pago = 'PROGRAMADO',
                      updated_by = ?,
                      updated_at = CURRENT_TIMESTAMP
                  WHERE id IN ($placeholders)";

        // El primer parámetro es el usuario, seguido de la lista de IDs de facturas
        $params = array_merge([$userId], $invoiceIds);

        return (bool) $this->update($query, $params);
    }
}